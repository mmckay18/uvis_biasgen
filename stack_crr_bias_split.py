...
data_cube1 = np.empty((nf, ny, nx), dtype=float)
data_cube2 = np.empty((nf, ny, nx), dtype=float)
err_data_cube1 = np.empty((nf, ny, nx), dtype=float)
err_data_cube2 = np.empty((nf, ny, nx), dtype=float)
#-------------------------
# Median Stack
#-------------------------
def median_stack1(j):
...
#Inputs the masked data in to the data cube
    ...
    print(err_chip2[1703:1708,2693:2698])
#    print (data_cube1[i,0:3,0:3])
#    print (np.shape(data_array_1))
    i+=1
    print(i)
...
new_hdul.writeto('combined_mean_stacked_file.fits',overwrite=True)
